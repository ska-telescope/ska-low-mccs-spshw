...
class SubarrayBeamHealthEvaluator(HardwareHealthEvaluator):
    ...
    subarray beam. A Subarray beam doesn't have hardware as such. Here
    we are pretending it does because we have to set health to DEGRADED
    if the beam is not locked, so for now we pretend that the
...
class SubarrayBeamHardwareManager(HardwareManager):
    """
    This class manages subarray beam "hardware".  A subarray beam
    doesn't actually have hardware. Here we are shoe-horning the
    subarray beam implementation into the hardware model by pretending
    that the `isLocked` attribute is a hardware property.
