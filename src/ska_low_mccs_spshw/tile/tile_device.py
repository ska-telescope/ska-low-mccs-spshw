...
class MccsTile(MccsBaseDevice[TileComponentManager]):
    ...
    class LoadCalibrationCoefficientsForChannelsCommand(FastCommand):
        """Class for LoadCalibrationCoefficientsForChannels command."""
        ...
        def __init__(
            ...
            """
            Initialise new LoadCalibrationCoefficientsForChannelsCommand.
        ...
        def do(  # type: ignore[override]
            ...
            **kwargs: Any,
            ...
            """
            Implement LoadCalibrationCoefficientsForChannels command.

            :param argin: flat list with first_channel, then calibration
                coefficients as flattened real/imag pairs
            ...
            :raises ValueError: if the argin argument does not have the
                right structure or length
            """
            # Minimum: first_channel (1) + one channel of data (128)
            # 128 = 16 antennas × 4 polarizations × 2 values (real, imag)
            if len(argin) < 129:
                self.logger.error("Insufficient calibration coefficients")
                raise ValueError("Insufficient calibration coefficients")

            if int(argin[0]) < 0:
                self.logger.error("first_channel must be non-negative")
                raise ValueError("first_channel must be non-negative")

            # Validate data length is a multiple of 128 (one channel's worth)
            # 128 = 16 antennas × 4 polarizations × 2 values per complex number
            if (len(argin) - 1) % 128 != 0:
                self.logger.error(
                    f"Data length must be a multiple of 128. "
                    f"Got {len(argin) - 1} values after first_channel"
                )
                raise ValueError(
                    f"Data length must be a multiple of 128. "
                    f"Got {len(argin) - 1} values"
                )

            # Parse flat array into 3D: [channel, antenna, polarization]
            # num_channels = (total_values - 1) / 128
            # Shape: (num_channels, 16 antennas, 4 polarizations)
            calibration_coefficients = np.zeros(
                ((len(argin) - 1) // 128, 16, 4), dtype=complex
            )

            idx = 1  # Start after first_channel (argin[0])
            for channel in range(calibration_coefficients.shape[0]):
                for antenna in range(16):  # 16 antennas per tile
                    for pol in range(4):  # 4 polarization components
                        calibration_coefficients[channel, antenna, pol] = complex(
                            argin[idx], argin[idx + 1]
                        )
                        idx += 2

            try:
                self._component_manager.load_calibration_coefficients_for_channels(
                    first_channel=int(argin[0]),
                    calibration_coefficients=calibration_coefficients,
                )
            except (ValueError, RuntimeError, AttributeError) as e:
                self.logger.error(f"Failed to load calibration coefficients: {e}")
                return (
                    ResultCode.FAILED,
                    f"Failed to load calibration coefficients: {e}",
                )

            return (
                ResultCode.OK,
                "LoadCalibrationCoefficientsForChannels completed OK",
            )
    ...
    def LoadCalibrationCoefficientsForChannels(
        ...
        """
        Load calibration coefficients for all antennas, specific channels.

        This does not apply them - that is performed by ApplyCalibration.
        ...
        * first_channel - (int) First channel index at the beamformer
            output for which calibration coefficients are provided.
        * calibration_coefficients - [array] flattened complex array
            with shape [channel, antenna, polarization]. Data is ordered:
            channel → antenna → polarization. Each complex value is
            represented as two consecutive floats (real, imaginary).

            The number of channels is determined by the length of the data:
            num_channels = (len(argin) - 1) / 128

            * channel - (int) channel index (0 to num_channels-1)
            * antenna - (int) antenna index, ranging 0 to 15
            * polarization - (int) polarization index from 0 to 3:
            ...
            Each channel has 16 antennas × 4 polarizations × 2 values
            (real, imag) = 128 floats.
        ...
        >>> first_channel = 100
        >>> # Create coefficients for 2 channels, 16 antennas, 4 pols each
        ...
        >>>         ],
        >>>         # ... repeat for antennas 1-15
        >>>     ],
        >>>     # ... channel 1
        >>> ]
        >>> # Flatten to [real, imag, real, imag, ...]
        >>> flat = []
        >>> for channel in complex_coefficients:
        >>>     for antenna in channel:
        >>>         for coeff in antenna:
        >>>             flat.extend([coeff.real, coeff.imag])
        >>> input_array = [float(first_channel)]
        >>> input_array.extend(flat)
        >>> dp = tango.DeviceProxy("mccs/tile/01")
        >>> result = dp.command_inout(
        >>>     "LoadCalibrationCoefficientsForChannels", input_array
        >>> )
        """
