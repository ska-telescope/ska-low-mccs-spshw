...
class TestSubrackTileDaqIntegration:
    ...
    def test_subrack_tile_daq_integration(
        ...
        :param daq_device_state_changed_callback: _description_
        ...
        def turn_on_subrack():
            ...
            # supply device. Until this connection is established, it is in UNKNOWN
            # state.
            subrack_device_state_changed_callback.assert_next_change_event(
                tango.DevState.UNKNOWN
            )

            # The subrack device connects to its upstream power supply device and finds
            # that the subrack is turned off, so it transitions to OFF state
            subrack_device_state_changed_callback.assert_last_change_event(
                tango.DevState.OFF
            )
            assert subrack_device.tpm1PowerState == PowerState.NO_SUPPLY

            # The tile device receives a change event. Since the event indicates that
            # the subrack hardware is OFF, the tile has established that its TPM is not
            # powered, so it transitions to OFF state.
            tile_device_state_changed_callback.assert_next_change_event(
                tango.DevState.OFF
            )
            ...
            # The subrack device tells the upstream power supply to power the subrack
            # on. Once the upstream power supply has powered the subrack on, the
            # subrack device tries to establish a connection to the subrack. Until that
            # connection is established, it is in UNKNOWN state.

            # TODO: Subrack is going straight to ON and not transitioning through
            # UNKNOWN subrack_device_state_changed_callback.assert_next_change_event(
            #     tango.DevState.UNKNOWN
            # )
            ...
            # The tile device is notified that its subrack is on. It now has
            # communication with its TPM. The first thing it does is subscribe to change
            # events on the power mode of its TPM. It is informed that the TPM is turned
            # off, so it remains in OFF state
        ...
        def turn_on_tpm():
            ...
            # The tile device tells the subrack device to tell its subrack to power on
            # its TPM. This is done. The subrack device detects that the TPM is now on.
            ...
            # Verify On has been called on Tpm
            args = lrc_result_changed_callback.get_next_call()
            assert "_On" in args[0][1][0]
            # TODO: This next line requires MCCS-1148 to be fixed
            # assert args[0][1][1] == f'"Tile on completed"'
            ...
            assert (
                args[0][1][1]
                == f'"Subrack TPM {tpm_id} turn on tpm task has completed"'
            )
        ...
        def start_daq():
            ...
            daq_device.Start(
                task_callback=task_callback, callbacks=[processed_data_callback]
            )
