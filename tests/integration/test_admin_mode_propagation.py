...
class TestAdminModePropagation:
    ...
    def test_daq_mode_propagation(
        ...
        """
        Test AdminMode propagation for daq.

        :param sps_station_device: fixture that provides a
            :py:class:`tango.DeviceProxy` to the device under test, in a
            :py:class:`tango.test_context.DeviceTestContext`.
        :param daq_device: fixture that provides a
            :py:class:`tango.DeviceProxy` to the device under test, in a
            :py:class:`tango.test_context.DeviceTestContext`.
        :param change_event_callbacks: group of Tango change event
            callback with asynchronous support
        """
        daq_device.inheritModes = True
        ...
        daq_device.subscribe_event(
            "adminMode",
            tango.EventType.CHANGE_EVENT,
            change_event_callbacks["daq_adminMode"],
        )
        ...
        # a higher lookahead value
        change_event_callbacks.assert_change_event(
            "daq_adminMode", AdminMode.ONLINE, lookahead=2, consume_nonmatches=True
        )
        assert daq_device.adminMode == AdminMode.ONLINE

        sps_station_device.adminMode = AdminMode.OFFLINE
        change_event_callbacks.assert_change_event("daq_adminMode", AdminMode.OFFLINE)
        assert daq_device.adminMode == AdminMode.OFFLINE

        # Turn inheritance off by setting directly
        daq_device.adminMode = AdminMode.ENGINEERING
        change_event_callbacks.assert_change_event(
            "daq_adminMode", AdminMode.ENGINEERING
        )

        sps_station_device.adminMode = AdminMode.ONLINE
        change_event_callbacks["daq_adminMode"].assert_not_called()
        assert daq_device.adminMode == AdminMode.ENGINEERING

        # Go back to inheriting
        daq_device.inheritModes = True
        change_event_callbacks.assert_change_event("daq_adminMode", AdminMode.ONLINE)
        assert daq_device.adminMode == AdminMode.ONLINE
